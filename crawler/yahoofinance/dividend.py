...
def crawl(folder_name):
    """배당금 지급 데이터를 크롤링하는 함수

    Args:
        folder_name (str): 데이터들이 저장될 폴더의 이름을 지정하는 변수
    """
    if not os.path.exists(f'./dividends/{folder_name}'):
        os.makedirs(f'./dividends/{folder_name}')
    ...
    for element in tqdm(table):
        ...
        if not df.empty:
            df = pd.DataFrame({'BASE_DATE':df.index.strftime('%Y-%m-%d'), 'DIVIDENDS':df.values})
            df['ITEM_CODE'] = ticker
            df['ITEM_NAME'] = name
            ...
            df.to_csv(f'./dividends/{folder_name}/{ticker}.csv', index=False)

def insert(folder_name):
    """배당금 지급 데이터를 DB에 넣는 함수

    Args:
        folder_name (str): 데이터들이 저장되어 있는 폴더의 이름을 지정하는 변수
    """
    folder_name = "./dividends/" + folder_name
    load_dotenv()
    try:
    ...
    query = f"""
        INSERT INTO YAHOO_DIVIDENDS (BASE_DATE,DIVIDENDS,ITEM_CODE,ITEM_NAME,MARKET_DIV)
        VALUES (%s,%s,%s,%s,%s);
        """
